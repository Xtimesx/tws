...
uniq_name_reqex = /^[a-fA-F0-9\/]+\/[0-9a-zA-Z\-_]+\.(jpg|png|mp4|webm)$/
    ...
    when "POST" then
      ...
      if !(b=context.request.body).nil?
        ...
        if !b.empty?
          ...
          raise "src is not twitter image server" unless job_request.src =~ /^https?:\/\/(pbs|video)\.twimg\.com\/(amplify_video|media|tweet_video_thumb|tweet_video|ext_tw_video\/.*)\/[0-9a-zA-Z\-_]+\.(jpg|png|mp4|webm)$/
